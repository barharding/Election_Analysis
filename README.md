...
As we can see the election results script can generate results with out the risk of human error during tabulation.  It is repeatable and consistent.   The election results python script can be modified any number of ways to calculate a variety of election outcomes and for different types of elections.  For example, unchanged simply entering state names in place of county names in the source data file could give you popular vote results by state in a federal election.  Other ways this process can be enhanced would be to add a column to the data and call it state.  That way you could have an output which shows the popular vote by state and by county.  This script could be enhanced to ask a user for which year they wish to see the results and using a variable it could find the file name based on year and retreive the election results for the specified year.

Ideally, the script would be modified to ask the user to specify the type of election for which to generate results.  Types could include city elections, state elections, federal elections and then execute the appropriate code for each type.  In addition, based on type it could read and load specific files from a resource file specific to the type of election.  This would ensure the right files are loaded for the right scenario.  If additional columns are added new variables, list, and dictionaries will need to be initailized and for loops and conditional evaluations made to calculate results.
